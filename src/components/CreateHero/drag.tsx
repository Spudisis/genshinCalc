  ...
  return (
    <>
      {!drag ? (
        ...
        >
          <label htmlFor={id + 'image'} onMouseEnter={() => setfocusHrefImg(true)} onMouseLeave={() => setfocusHrefImg(false)}>
            Изображение*
          </label>
          <Field
            type='text'
          ...
          {ViewListHeroes && (
            <SelectHero setSelectImg={(n: string) => setSelectImg(n)} setViewListHeroes={(n: boolean) => setViewListHeroes(n)} />
          )}
        </div>
      ) : (
        <div
